...
l'application à travers les \gls{api} de la base de données \dev{SQLite}
...
Cette implémentation peut être subdivisé en trois éléments: Les \dev{Contrats}, les \dev{Helpers}, et la classe \dev{DBSetup}.
...
d'implémenté la méthode \dev{login()} qui prend en argument
...
\dev{getPhoto()} retourne respectivement le nom de l'utilisateur et sa
photo.
...
Dans le langage Java, ce patron est réaliser à travers la classe \dev{java.util.Observable} et l'interface \dev{Java.util.Observer}. Le Sujet hérite de la classe \dev{Observable} et les changements sont signalé par les méthodes \dev{setChanged()} et \dev{notifyObservers()} ou \dev{notifyObservers(Object message)}.
...
\android{} \en{Lint} (figure \ref{fig:lint}) est un outil introduit dans la version 16 de \gls{adt} qui scanne les code sources des projet \android{} afin d'y détecter des mal-fonctions potentiels.
